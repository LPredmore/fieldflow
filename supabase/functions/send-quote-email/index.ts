...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // Generate share token and update quote
    ...
    const { data: quote, error: quoteError } = await supabase
      .from('quotes')
      .select('quote_number, title, total_amount')
      .eq('id', quoteId)
      .single();
    ...
    const publicUrl = `${Deno.env.get("SUPABASE_URL")?.replace('/auth/v1', '')}/public-quote/${shareToken}`;
    ...
    const emailResponse = await resend.emails.send({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <p>We're pleased to provide you with a quote for <strong>${quote.title}</strong>.</p>
